...
def apply_filters(
    ...
    if filter_expr:
        if "=" not in filter_expr:
            print(c("red", f"  ✗ Invalid filter '{filter_expr}'. Use field=value."))
        else:
            key, _, val = filter_expr.partition("=")
            rows = [r for r in rows if str(r.get(key.strip(), "")).lower() == val.strip().lower()]
...
def main() -> None:
    ...
    parser.add_argument("--filter",  "-f", metavar="FIELD=VALUE", help="Filter rows, e.g. --filter status=up")
    ...
    parser.add_argument("--columns", "-c", metavar="COL1,COL2",   help="Show only specific columns (comma-separated)")
